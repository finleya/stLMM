...
random effects. The goal is comparison, not replacement. `lme4` is a mature and
general mixed-model package. Bates, Maechler, Bolker, and Walker
[@BatesMaechlerBolkerWalker2015lme4] describe `lme4` as a modular system for
fitting linear and generalized linear mixed models using sparse-matrix methods
and profiled likelihood or REML estimation.
...
- `stLMM` currently represents Poisson models through a large-size
  negative-binomial approximation, not a native Poisson likelihood;
- `stLMM` fixes the negative-binomial size parameter, while
...
the same linear mixed-model likelihood for the independent random effects used
here.
...
variance and the two independent iid random-effect variances. We do not provide
starting values; `stLMM` uses its defaults. We run three chains so the posterior
summaries can be read with the accompanying $\hat R$ diagnostics. The `lme4` fit
uses REML.
