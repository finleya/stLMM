...
$$
\theta_{a,t} =
\operatorname{avg}_{g \in U_{a,t}} y_g,
$$

where $U_{a,t}$ is the county-year prediction support. In this vignette, the grid defines the aggregation support for the unit-response summaries. The unobserved grid-row values $y_g$ are represented by model-based predictions $\tilde{y}_g$.
...
The 10 km grid rows define the county-year prediction support. For each fitted model, posterior predictive draws are averaged over grid rows within each county-year.
...
The county panels are a useful check on the unit-response summaries because they show how the single-stage and two-stage models respond to sparse county-year direct estimates. In Yakima, the unit-response summaries are consistently lower than many of the direct estimates. We return to this pattern in the next vignette, where the model is better matched to the county-time structure in the data and the lower Yakima predictions persist.
