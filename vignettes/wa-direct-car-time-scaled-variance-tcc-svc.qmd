...
$$
\theta_i = \beta_0 + \{\beta_1 + u_{a_i}\}x_i + w_{a_i,t_i},
$$

where $x_i$ is standardized county mean TCC, $u_a$ is a county-level CAR deviation from the global TCC slope, and $w_{a,t}$ is a separable CAR-time process over counties and years. Both CAR components use the county graph; the CAR-time term also uses the annual support.
...
County mean TCC is computed from the full annual TCC raster for each county-year. The model uses a standardized version of this covariate so the global TCC coefficient and county-specific deviations are measured per one standard deviation increase in county mean TCC.
...
The caveat is that TCC is itself strongly spatially patterned. In this dataset, low TCC, low biomass, and eastern counties often occur together, so the SVC can be partly confounded with broad east-west forest/nonforest structure. That makes the model useful for teaching and exploration, but the coefficient surface should be interpreted with diagnostic care.

WAIC can be used to compare this model with the global-TCC scaled-variance model because both use the same likelihood rows. This article leaves that comparison to the series bake-off so the focus stays on the modeling extension itself.
